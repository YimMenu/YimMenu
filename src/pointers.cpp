...
namespace big
{
	void pointers::get_gta_batch(memory::module region)
	{
		...
		// Screen Resolution
		main_batch.add("SR", "66 0F 6E 0D ? ? ? ? 0F B7 3D", [](memory::handle ptr) {
			g_pointers->m_gta.m_resolution_x = ptr.sub(4).rip().as<int*>();
			g_pointers->m_gta.m_resolution_y = ptr.add(4).rip().as<int*>();
		});

		// Region Code
		main_batch.add("RC", "48 83 EC 28 83 3D ? ? ? ? ? 75 10", [](memory::handle ptr) {
			g_pointers->m_gta.m_region_code = ptr.add(16).rip().add(1).as<uint32_t*>();
		});

		// Ocean Quads
		main_batch.add("OQ", "74 41 4C 8B 05 ? ? ?", [](memory::handle ptr) {
			g_pointers->m_gta.m_ocean_quads = ptr.add(5).rip().as<uint64_t>();
		});

		// Gravity Level
		main_batch.add("GL", "48 8D 0D ? ? ? ? F3 0F 10 04 81 F3 0F 11 05", [](memory::handle ptr) {
			g_pointers->m_gta.m_gravity_level = ptr.add(3).rip().as<float*>();
		});

		// Set Gravity Level
		main_batch.add("SGL", "48 83 EC ? 83 F9 ? 77 ? 48 63 C1 48 8D 0D", [](memory::handle ptr) {
			g_pointers->m_gta.m_set_gravity_level = ptr.as<functions::set_gravity_level>();
		});

		// Game State
		main_batch.add("GS", "83 3D ? ? ? ? ? 75 17 8B 43 20 25", [](memory::handle ptr) {
			g_pointers->m_gta.m_game_state = ptr.add(2).rip().add(1).as<eGameState*>();
		});

		// Is Session Started
		main_batch.add("ISA", "40 38 35 ? ? ? ? 75 0E 4C 8B C3 49 8B D7 49 8B CE", [](memory::handle ptr) {
			g_pointers->m_gta.m_is_session_started = ptr.add(3).rip().as<bool*>();
		});

		// Ped Factory
		main_batch.add("PF", "48 8B 05 ? ? ? ? 48 8B 48 08 48 85 C9 74 52 8B 81", [](memory::handle ptr) {
			g_pointers->m_gta.m_ped_factory = ptr.add(3).rip().as<CPedFactory**>();
		});

		// Network Player Manager
		main_batch.add("NPM", "48 8B 0D ? ? ? ? 8A D3 48 8B 01 FF 50 ? 4C 8B 07 48 8B CF", [](memory::handle ptr) {
			g_pointers->m_gta.m_network_player_mgr = ptr.add(3).rip().as<CNetworkPlayerMgr**>();
		});

		// Init Native Tables & Native Handlers
		main_batch.add("NH", "48 8D 0D ? ? ? ? 48 8B 14 FA E8 ? ? ? ? 48 85 C0 75 0A", [](memory::handle ptr) {
		...
		// Fix Vectors
		main_batch.add("FV", "83 79 18 00 48 8B D1 74 4A FF 4A 18 48 63 4A 18 48 8D 41 04 48 8B 4C CA", [](memory::handle ptr) {
			g_pointers->m_gta.m_fix_vectors = ptr.as<functions::fix_vectors>();
		});

		// Script Threads
		main_batch.add("ST", "45 33 F6 8B E9 85 C9 B8", [](memory::handle ptr) {
		...
		// Script Programs
		main_batch.add("SP", "48 8B 1D ? ? ? ? 41 83 F8 FF", [](memory::handle ptr) {
			g_pointers->m_gta.m_script_program_table = ptr.add(3).rip().as<decltype(gta_pointers::m_script_program_table)>();
		});

		// Script Global
		main_batch.add("SG", "48 8D 15 ? ? ? ? 4C 8B C0 E8 ? ? ? ? 48 85 FF 48 89 1D", [](memory::handle ptr) {
			g_pointers->m_gta.m_script_globals = ptr.add(3).rip().as<std::int64_t**>();
		});

		// Game Script Handle Manager
		main_batch.add("CGSHM", "48 8B 0D ? ? ? ? 4C 8B CE E8 ? ? ? ? 48 85 C0 74 05 40 32 FF", [](memory::handle ptr) {
		...
		main_batch.add("WMSB", "48 85 C0 0F 84 ? ? ? ? 8B 48 50", [](memory::handle ptr) {
			g_pointers->m_gta.m_world_model_spawn_bypass = ptr.as<PVOID>();
		});

		// Native Return Spoofer
		main_batch.add("NRF", "FF E3", [](memory::handle ptr) {
			g_pointers->m_gta.m_native_return = ptr.add(0).as<PVOID>();
		});

		// GTA Thread Start
		main_batch.add("GTS", "48 89 5C 24 ? 48 89 74 24 ? 57 48 83 EC 20 8B FA 85 D2 75 2A 8B 15", [](memory::handle ptr) {
			g_pointers->m_gta.m_gta_thread_start = ptr.as<PVOID>();
		});

		// GTA Thread Kill
		main_batch.add("TK", "48 89 5C 24 ? 57 48 83 EC 20 48 83 B9 ? ? ? ? ? 48 8B D9 74 14", [](memory::handle ptr) {
		...
		// Handle To Ptr
		main_batch.add("GSH", "83 F9 FF 74 31 4C 8B 0D", [](memory::handle ptr) {
			g_pointers->m_gta.m_handle_to_ptr = ptr.as<decltype(gta_pointers::m_handle_to_ptr)>();
		});

		// CNetworkObjectMgr
		main_batch.add("CNOM", "48 8B 0D ? ? ? ? 45 33 C0 E8 ? ? ? ? 33 FF 4C 8B F0", [](memory::handle ptr) {
			g_pointers->m_gta.m_network_object_mgr = ptr.add(3).rip().as<CNetworkObjectMgr**>();
		});

		// Network Player Mgr Init
		main_batch.add("NPMI", "41 56 48 83 EC ? 48 8B F1 B9 ? ? ? ? 49 8B F9 41 8B E8 4C 8B F2 E8", [](memory::handle ptr) {
		...
		main_batch.add("NPMS", "48 8D 9F ? ? ? ? EB ? 48 8B 13 48 85 D2 74 ? 48 8B CB E8 ? ? ? ? 48 83 7B ? ? 75 ? 48 8D 9F", [](memory::handle ptr) {
			g_pointers->m_gta.m_network_player_mgr_shutdown = ptr.sub(0x1A).as<decltype(gta_pointers::m_network_player_mgr_shutdown)>();
		});

		// FriendRegistry
		main_batch.add("FR", "3B 0D ? ? ? ? 73 17", [](memory::handle ptr) {
			g_pointers->m_gta.m_friend_registry = ptr.add(2).rip().as<FriendRegistry*>();
		});

		// GET_SCREEN_COORDS_FROM_WORLD_COORDS
		main_batch.add("GSCFWC", "E8 ? ? ? ? 84 C0 74 19 F3 0F 10 44 24", [](memory::handle ptr) {
		...
		main_batch.add("GGCC", "8B 90 ? ? ? ? 89 13", [](memory::handle ptr) {
		...
		// Receive Net Message
		main_batch.add("RNM", "48 83 EC 20 4C 8B 71 50 33 ED", [](memory::handle ptr) {
			g_pointers->m_gta.m_receive_net_message = ptr.sub(0x19).as<PVOID>();
		});

		// Get Network Event Data
		main_batch.add("GNED", "53 43 52 49 50 54 5F 4E 45 54 57 4F 52 4B", [](memory::handle ptr) {
			g_pointers->m_gta.m_get_network_event_data = *ptr.sub(0x38).as<PVOID*>();
		});

		// Assign Physical Index
		main_batch.add("API", "48 89 5C 24 ? 48 89 6C 24 ? 48 89 74 24 ? 57 41 54 41 55 41 56 41 57 48 83 EC 20 41 8A E8", [](memory::handle ptr) {
			g_pointers->m_gta.m_assign_physical_index = ptr.as<PVOID>();
		});

		// Received Clone Create
		main_batch.add("RCC", "48 8B C4 66 44 89 48", [](memory::handle ptr) {
			g_pointers->m_gta.m_received_clone_create = ptr.as<PVOID>();
		});

		// Can Apply Data
		main_batch.add("CAD", "E8 ? ? ? ? 84 C0 0F 84 AF 01 00 00 48 8B 03", [](memory::handle ptr) {
		...
		main_batch.add("RCS/GSTFT/GNOFP/GNO/GSTI", "48 89 5C 24 08 48 89 6C 24 10 48 89 74 24 18 57 41 54 41 55 41 56 41 57 48 83 EC 40 4C 8B EA", [](memory::handle ptr) {
		...
		// Model Hash Table
		main_batch.add("MHT", "4C 03 05 ? ? ? ? EB 03", [](memory::handle ptr) {
			g_pointers->m_gta.m_model_table = ptr.add(3).rip().as<HashTable<CBaseModelInfo*>*>();
		});

		// Get Model Info
		main_batch.add("GMI", "41 3B 0A 74 54", [](memory::handle ptr) {
		...
		main_batch.add("RENC", "E8 ? ? ? ? 8B 8B ? ? ? ? 03 CF", [](memory::handle ptr) {
		...
		// fiPackfile ctor
		main_batch.add("FPFC", "44 89 41 28 4C 89 41 38 4C 89 41 50 48 8D", [](memory::handle ptr) {
		...
		// fiPackfile dtor
		main_batch.add("FPFD", "48 89 5C 24 08 57 48 83 EC 20 48 8D 05 ? ? ? ? 33 FF 48 8B D9 48 89 01 40 88", [](memory::handle ptr) {
			g_pointers->m_gta.m_fipackfile_dtor = ptr.as<functions::fipackfile_dtor>();
		});

		// fiPackfile stuff
		main_batch.add("FPU&FPCA&FPOA&FPM", "E8 ? ? ? ? 48 8D 0D ? ? ? ? E8 ? ? ? ? 8A 05 ? ? ? ? 48 8D 0D", [](memory::handle ptr) {
		...
		main_batch.add("IMCD", "E8 ? ? ? ? 40 88 7C 24 ? 49 89 9C 24", [](memory::handle ptr) {
			g_pointers->m_gta.m_invalid_mods_crash_detour = ptr.add(1).rip().as<PVOID>();
		});

		// Send Chat Ptr
		main_batch.add("SCP", "83 7E 1C 01 48 8B 3D", [](memory::handle ptr) {
			g_pointers->m_gta.m_send_chat_ptr = ptr.add(7).rip().as<int64_t**>();
		});

		// Send Chat Message
		main_batch.add("SCM", "48 83 EC 20 48 8B F1 48 8B CA 41 8A E9", [](memory::handle ptr) {
			g_pointers->m_gta.m_send_chat_message = ptr.sub(21).as<functions::send_chat_message>();
		});

		// Get Gamer Online State
		main_batch.add("GGOS", "48 8B 44 24 70 44 8B CD 4D 8B C6 41 8B D7 48 8B CF 48 89 47 40", [](memory::handle ptr) {
		...
		main_batch.add("SGSBGH", "E8 ? ? ? ? 84 C0 0F 84 ? ? ? ? 8B 05 ? ? ? ? 48 8D 4C 24", [](memory::handle ptr) {
		...
		main_batch.add("SGSBGH", "E8 ? ? ? ? 84 C0 0F 84 F6 FE FF FF", [](memory::handle ptr) {
		...
		main_batch.add("JSBI", "E8 ? ? ? ? 0F B6 CB 84 C0 41 0F 44 CD", [](memory::handle ptr) {
			g_pointers->m_gta.m_join_session_by_info = ptr.add(1).rip().as<functions::join_session_by_info>();
		});

		// Invite Player By Gamer Handle
		main_batch.add("IPBGH", "E8 ? ? ? ? 4C 8D 05 ? ? ? ? 48 8D 15 ? ? ? ? E9", [](memory::handle ptr) {
		...
		main_batch.add("AFBGH", "48 89 5C 24 ? 57 48 83 EC ? 48 8B F9 B1 ? 48 8B DA E8 ? ? ? ? 84 C0 74 ? 8B 15", [](memory::handle ptr) {
		...
		// Send Network Damage
		main_batch.add("SND", "E8 ? ? ? ? E9 E9 01 00 00 48 8B CB", [](memory::handle ptr) {
			g_pointers->m_gta.m_send_network_damage = ptr.add(1).rip().as<functions::send_network_damage>();
		});

		// Request Ragdoll
		main_batch.add("RR", "E8 ? ? ? ? 09 B3 ? ? ? ? 48 8B 5C 24", [](memory::handle ptr) {
			g_pointers->m_gta.m_request_ragdoll = ptr.add(1).rip().as<functions::request_ragdoll>();
		});

		// Request Control
		main_batch.add("RC", "E8 ? ? ? ? EB 3E 48 8B D3", [](memory::handle ptr) {
			g_pointers->m_gta.m_request_control = ptr.add(1).rip().as<functions::request_control>();
		});

		// Send Remove Gamer Command
		main_batch.add("SRGC", "8D 42 FF 83 F8 FD 77 3D", [](memory::handle ptr) {
			g_pointers->m_gta.m_send_remove_gamer_cmd = ptr.add(65).rip().as<functions::send_remove_gamer_cmd>();
		});

		// Get Connection Peer
		main_batch.add("GCP", "48 89 5C 24 08 48 89 74 24 18 89 54 24 10 57 48 83 EC 40 48", [](memory::handle ptr) {
			g_pointers->m_gta.m_get_connection_peer = ptr.as<functions::get_connection_peer>();
		});

		// Handle Remove Gamer Command
		main_batch.add("HRGC", "48 85 D2 0F 84 0E 04", [](memory::handle ptr) {
			g_pointers->m_gta.m_handle_remove_gamer_cmd = ptr.as<functions::handle_remove_gamer_cmd>();
		});

		// Broadcast Net Array
		main_batch.add("BNA", "48 89 5C 24 ? 48 89 54 24 ? 55 56 57 41 54 41 55 41 56 41 57 48 83 EC 40 48 8B 05 ? ? ? ? 66 44 89 4C 24", [](memory::handle ptr) {
			g_pointers->m_gta.m_broadcast_net_array = ptr.as<PVOID>();
		});

		// Rage Security
		main_batch.add("RS", "48 8B 1D ? ? ? ? 33 F6 BD C3 9E 26 00", [](memory::handle ptr) {
			g_pointers->m_gta.m_security = ptr.add(3).rip().as<rage::atSingleton<rage::RageSecurity>*>();
		});

		// Send Session Matchmaking Attributes
		main_batch.add("SSMA", "48 8B C4 48 89 58 08 48 89 68 10 48 89 70 18 48 89 78 20 41 56 48 81 EC D0 00 00 00 49 8B", [](memory::handle ptr) {
			g_pointers->m_gta.m_send_session_matchmaking_attributes = ptr.as<PVOID>();
		});

		// Serialize Take Off Ped Variation Task
		main_batch.add("STOPVT", "40 55 53 57 41 56 48 8B EC 48 83 EC 68", [](memory::handle ptr) {
			g_pointers->m_gta.m_serialize_take_off_ped_variation_task = ptr.as<PVOID>();
		});

		// Serialize Parachute Task
		main_batch.add("SPT", "40 55 53 56 57 41 54 48 8B", [](memory::handle ptr) {
			g_pointers->m_gta.m_serialize_parachute_task = ptr.as<PVOID>();
		});

		// Chat Data
		main_batch.add("CD", "48 8B 05 ? ? ? ? 0F 45 DF", [](memory::handle ptr) {
			g_pointers->m_gta.m_chat_data = ptr.add(3).rip().as<ChatData**>();
		});

		// Social Club Info
		main_batch.add("SCI", "48 8B D3 48 8D 4C 24 ? 48 69 D2", [](memory::handle ptr) {
			g_pointers->m_gta.m_sc_info = ptr.sub(4).rip().as<ScInfo*>();
		});

		// Invalid Decal Crash
		main_batch.add("IDC", "E8 ? ? ? ? 8B 9C 24 B8 00 00 00 4C 8B AC 24 A8 00 00 00", [](memory::handle ptr) {
			g_pointers->m_gta.m_invalid_decal_crash = ptr.add(1).rip().as<PVOID>();
		});

		// Task Parachute Object
		main_batch.add("TPO", "0F 88 ? ? ? ? 75 34", [](memory::handle ptr) {
			g_pointers->m_gta.m_task_parachute_object = ptr.sub(6).as<PVOID>();
		});

		// Task Ambient Clips
		main_batch.add("TAC", "48 89 5C 24 08 48 89 74 24 10 57 48 83 EC 20 80 3D ? ? ? ? ? 41 8B D8 8B F2", [](memory::handle ptr) {
			g_pointers->m_gta.m_task_ambient_clips = ptr.as<PVOID>();
		});

		// Encode Session Info
		main_batch.add("ESI", "E8 ? ? ? ? C6 83 94 01 00 00 01", [](memory::handle ptr) {
			g_pointers->m_gta.m_encode_session_info = ptr.add(1).rip().as<functions::encode_session_info>();
		});

		// Decode Session Info
		main_batch.add("DSI", "E8 ? ? ? ? 84 C0 74 16 48 8B 4B 60", [](memory::handle ptr) {
			g_pointers->m_gta.m_decode_session_info = ptr.add(1).rip().as<functions::decode_session_info>();
		});

		// Decode Peer Info
		main_batch.add("DPI", "48 89 5C 24 08 48 89 74 24 10 57 48 81 EC C0 00 00 00 48 8B F1 49", [](memory::handle ptr) {
			g_pointers->m_gta.m_decode_peer_info = ptr.as<functions::decode_peer_info>();
		});

		// Main File Object
		main_batch.add("MFO", "48 8D 05 ? ? ? ? 48 8D 1C D0 EB 03", [](memory::handle ptr) {
			g_pointers->m_gta.m_main_file_object = ptr.add(3).rip().as<datafile_commands::SveFileObject*>();
		});

		// Load Cloud File
		main_batch.add("LCF", "48 89 5C 24 08 48 89 74 24 10 57 48 83 EC 40 48 8B F2 48 8B D9 41 8B D0", [](memory::handle ptr) {
			g_pointers->m_gta.m_load_cloud_file = ptr.as<functions::load_cloud_file>();
		});

		// Set As Active Cloud File
		main_batch.add("SAACF", "48 83 EC 28 45 33 C0 4C 39", [](memory::handle ptr) {
			g_pointers->m_gta.m_set_as_active_cloud_file = ptr.as<functions::set_as_active_cloud_file>();
		});

		// Save JSON Data
		main_batch.add("SJD", "48 89 5C 24 08 57 48 83 EC 30 33 DB 48 8B FA 48", [](memory::handle ptr) {
			g_pointers->m_gta.m_save_json_data = ptr.as<functions::save_json_data>();
		});

		// Network Time
		main_batch.add("NT", "48 8B 0D ? ? ? ? E8 ? ? ? ? 33 DB 84 C0 74 41", [](memory::handle ptr) {
			g_pointers->m_gta.m_network_time = ptr.add(3).rip().as<rage::netTime**>();
		});

		// Sync Network Time
		main_batch.add("SNT", "E8 ? ? ? ? 8B 43 5C", [](memory::handle ptr) {
			g_pointers->m_gta.m_sync_network_time = ptr.add(1).rip().as<functions::sync_network_time>();
		});

		// Queue Dependency
		main_batch.add("QD", "48 89 5C 24 08 48 89 74 24 10 57 48 83 EC 20 8B F2 49 8B F8", [](memory::handle ptr) {
			g_pointers->m_gta.m_queue_dependency = ptr.as<PVOID>();
		});

		// Interval Check Function
		main_batch.add("ICF", "48 8D 0D ? ? ? ? 88 05 ? ? ? ? 48 8D 05", [](memory::handle ptr) {
			g_pointers->m_gta.m_interval_check_func = ptr.add(3).rip().as<PVOID>();
		});

		// Prepare Metric For Sending
		main_batch.add("PMFS", "48 8B C4 48 89 58 08 48 89 68 10 48 89 70 18 48 89 78 20 41 56 48 83 EC 30 49 8B E8 4C 8D 40 EC 49 8B F1 48 8B D9 40 32 FF E8", [](memory::handle ptr) {
			g_pointers->m_gta.m_prepare_metric_for_sending = ptr.as<PVOID>();
		});

		// HTTP Start Request
		main_batch.add("HSR", "48 89 5C 24 ? 48 89 74 24 ? 57 48 83 EC 20 48 8B D9 48 81 C1 ? ? ? ? 48 8B F2 33 FF E8", [](memory::handle ptr) {
			g_pointers->m_gta.m_http_start_request = ptr.as<PVOID>();
		});

		// Send Packet
		main_batch.add("SP", "48 8B C4 48 89 58 08 48 89 70 10 48 89 78 18 4C 89 48 20 55 41 54 41 55 41 56 41 57 48 8D A8 98", [](memory::handle ptr) {
			g_pointers->m_gta.m_send_packet = ptr.as<functions::send_packet>();
		});

		// Connect To Peer
		main_batch.add("CTP", "48 89 5C 24 08 4C 89 4C 24 20 48 89 54 24 10 55 56 57 41 54 41 55 41 56 41 57 48 83 EC 60 4D", [](memory::handle ptr) {
			g_pointers->m_gta.m_connect_to_peer = ptr.as<functions::connect_to_peer>();
		});

		// Fragment Physics Crash 2
		main_batch.add("FPC2", "E8 ? ? ? ? 84 C0 75 0B 41 FF CF", [](memory::handle ptr) {
			g_pointers->m_gta.m_fragment_physics_crash_2 = ptr.add(1).rip().as<PVOID>();
		});

		// Clear Ped Tasks Network
		main_batch.add("CPTN", "E8 ? ? ? ? EB 28 48 8B 8F A0 10 00 00", [](memory::handle ptr) {
			g_pointers->m_gta.m_clear_ped_tasks_network = ptr.add(1).rip().as<functions::clear_ped_tasks_network>();
		});

		// Get Entity Attached To
		main_batch.add("GEAT", "48 83 EC 28 48 8B 51 50 48 85 D2 74 04", [](memory::handle ptr) {
			g_pointers->m_gta.m_get_entity_attached_to = ptr.as<functions::get_entity_attached_to>();
		});

		// Received Array Update
		main_batch.add("RAU", "48 89 5C 24 10 55 56 57 41 54 41 55 41 56 41 57 48 8B EC 48 83 EC 30 48 8B 05", [](memory::handle ptr) {
			g_pointers->m_gta.m_received_array_update = ptr.as<PVOID>();
		});

		// Receive Pickup
		main_batch.add("RPI", "49 8B 80 ? ? ? ? 48 85 C0 74 0C F6 80 ? ? ? ? ? 75 03 32 C0 C3", [](memory::handle ptr) {
		...
		main_batch.add("EAD", "48 89 5C 24 08 48 89 7C 24 10 55 48 8B EC 48 83 EC 70 41", [](memory::handle ptr) {
			g_pointers->m_gta.m_enumerate_audio_devices = ptr.as<PVOID>();
		});

		// Direct Sound Capture Create
		main_batch.add("DSCC", "E8 ? ? ? ? 33 FF 85 C0 78 C1", [](memory::handle ptr) {
			g_pointers->m_gta.m_direct_sound_capture_create = ptr.add(1).rip().as<PVOID>();
		});

		// Refresh Audio Input
		main_batch.add("RAI", "40 88 3D ? ? ? ? 89 05 ? ? ? ? 40 38 3D", [](memory::handle ptr) {
		...
		main_batch.add("AWIV", "49 8B 40 08 39 10", [](memory::handle ptr) {
			g_pointers->m_gta.m_allow_weapons_in_vehicle = ptr.sub(23).as<PVOID>();
		});

		// Write Vehicle Proximity Migration Data Node
		main_batch.add("WVPMDN", "48 89 4C 24 08 55 53 56 57 41 54 41 55 41 56 41 57 48 8B EC 48 83 EC 68 4C 8B A9", [](memory::handle ptr) {
			g_pointers->m_gta.m_write_vehicle_proximity_migration_data_node = ptr.as<PVOID>();
		});

		// Migrate Object
		main_batch.add("MO", "48 89 5C 24 08 48 89 6C 24 10 48 89 74 24 18 57 41 54 41 55 41 56 41 57 48 83 EC 20 41 8B F8 48", [](memory::handle ptr) {
			g_pointers->m_gta.m_migrate_object = ptr.as<functions::migrate_object>();
		});

		// Game Version + Online Version
		main_batch.add("GVOV", "8B C3 33 D2 C6 44 24 20", [](memory::handle ptr) {
		...
		main_batch.add("TJC", "48 89 5C 24 ? 89 54 24 10 57 48 83 EC 30 0F 29 74 24", [](memory::handle ptr) {
		...
		main_batch.add("GHAHBI", "48 89 5C 24 08 48 89 6C 24 10 48 89 74 24 18 57 48 83 EC 20 8A 81 8F", [](memory::handle ptr) {
		...
		main_batch.add("SNPPD", "E8 ? ? ? ? 4C 8B 0F 44 0F B7 85 A0 01 00 00", [](memory::handle ptr) {
			g_pointers->m_gta.m_send_non_physical_player_data = ptr.add(1).rip().as<PVOID>();
		});

		// Presence Data
		main_batch.add("PD", "48 8B 0D ? ? ? ? 44 8B 4B 60", [](memory::handle ptr) {
			g_pointers->m_gta.m_presence_data = ptr.add(3).rip().as<void**>();
		});

		// Allocate Memory Reliable & Connection Manager Try Free Memory
		main_batch.add("AMR&CMTFM", "48 8B C4 48 89 58 08 48 89 68 10 48 89 70 18 48 89 78 20 41 56 48 83 EC 20 48 8B D9 48 8B 49 18", [](memory::handle ptr) {
		...
		main_batch.add("RMFQ&RMFUR", "E8 ? ? ? ? 0F B7 43 4C 48 8D 55 20", [](memory::handle ptr) {
		...
		main_batch.add("DHM", "48 89 05 ? ? ? ? EB 07 48 89 1D ? ? ? ? 48 8B CB", [](memory::handle ptr) {
			g_pointers->m_gta.m_draw_handler_mgr = ptr.add(3).rip().as<PVOID*>();
		});

		// Render Ped
		main_batch.add("RP", "48 89 5C 24 08 48 89 6C 24 10 48 89 74 24 18 57 41 54 41 55 41 56 41 57 48 81 EC 80 00 00 00 48 8B FA", [](memory::handle ptr) {
			g_pointers->m_gta.m_render_ped = ptr.as<PVOID*>();
		});

		// Render Entity
		main_batch.add("RE", "48 8B C4 48 89 58 08 48 89 68 10 48 89 70 18 57 41 54 41 55 41 56 41 57 48 83 EC 70 0F BA", [](memory::handle ptr) {
			g_pointers->m_gta.m_render_entity = ptr.as<PVOID*>();
		});

		// Render Big Ped
		main_batch.add("RE", "48 89 5C 24 08 4C 89 44 24 18 55 56 57 41 54 41 55 41 56 41 57 48 8B EC 48 81 EC 80 00 00 00 48", [](memory::handle ptr) {
			g_pointers->m_gta.m_render_big_ped = ptr.as<PVOID*>();
		});

		// Force Relay Connections
		main_batch.add("FRC", "8A 05 ? ? ? ? 88 83 BC 00 00 00", [](memory::handle ptr) {
			g_pointers->m_gta.m_force_relay_connections = ptr.add(2).rip().as<bool*>();
		});

		// Read Bits Single
		main_batch.add("RBS", "48 8B C4 48 89 58 08 48 89 68 10 48 89 70 18 48 89 78 20 41 56 41 57 33 FF", [](memory::handle ptr) {
			g_pointers->m_gta.m_read_bits_single = ptr.as<PVOID*>();
		});

		// Remove Reference
		main_batch.add("RR", "48 89 5C 24 08 57 48 83 EC 20 80 3D ? ? ? ? ? 48 8B FA 48 8B D9 74 13", [](memory::handle ptr) {
			g_pointers->m_gta.m_remove_reference = ptr.as<functions::remove_reference>();
		});

		// Sync Data Reader Vtable
		main_batch.add("RBS", "48 8D 05 ? ? ? ? 48 8D 54 24 20 48 89 44 24 20 48 8D 44 24 40", [](memory::handle ptr) {
			g_pointers->m_gta.m_sync_data_reader_vtable = ptr.add(3).rip().as<void**>();
		});

		// Interior Proxy Pool
		main_batch.add("IPP", "4C 8B 05 ? ? ? ? 4C 0F BF 0B", [](memory::handle ptr) {
			g_pointers->m_gta.m_interior_proxy_pool = ptr.add(3).rip().as<GenericPool**>();
		});

		// Train Config Array
		main_batch.add("TCA", "48 8D 0D ? ? ? ? E8 ? ? ? ? 44 88 64 24 30 4C 8D 8C 24 60 02 00 00", [](memory::handle ptr) {
			g_pointers->m_gta.m_train_config_array = ptr.add(3).rip().as<rage::atArray<CTrainConfig>*>();
		});

		// Activate Special Ability
		main_batch.add("ASA", "48 89 5C 24 08 48 89 74 24 10 57 48 83 EC 20 33 DB 8B F2 48 8B F9 48 39 99 D0", [](memory::handle ptr) {
			g_pointers->m_gta.m_activate_special_ability = ptr.as<functions::activate_special_ability>();
		});

		// Set Wanted Level
		main_batch.add("SWL", "48 8B C4 48 89 58 08 48 89 68 10 48 89 70 18 44 88 48 20 57 41 54 41 55 41 56 41 57 48 83 EC 30 4C 8B F1 48 8B 0D ? ? ? ? 44 8B E2", [](memory::handle ptr) {
			g_pointers->m_gta.m_set_wanted_level = ptr.as<functions::set_wanted_level>();
		});

		// Event Stuff
		main_batch.add("NEM&CEQ&NEP&GNPI&QNE", "48 39 99 D0 00 00 00 74 3C", [](memory::handle ptr) {
		...
		main_batch.add("CDBE", "48 89 5C 24 08 57 48 83 EC 30 33 FF BA 1B", [](memory::handle ptr) {
			g_pointers->m_gta.m_construct_door_break_event = ptr.as<functions::construct_door_break_event>();
		});

		// Delete Ped
		main_batch.add("DP", "48 83 EC 28 48 85 C9 74 12 48", [](memory::handle ptr) {
			g_pointers->m_gta.m_delete_ped = ptr.as<functions::delete_ped>();
		});

		// Delete Vehicle
		main_batch.add("DV", "48 85 C9 74 38 53 48 83 EC 20 80", [](memory::handle ptr) {
			g_pointers->m_gta.m_delete_vehicle = ptr.as<functions::delete_vehicle>();
		});

		// Delete Object
		main_batch.add("DO", "48 8B C4 48 89 58 08 48 89 68 10 48 89 70 18 48 89 78 20 41 56 48 83 EC 30 45 33 F6 40", [](memory::handle ptr) {
			g_pointers->m_gta.m_delete_object = ptr.as<functions::delete_object>();
		});

		// Max Wanted Level
		main_batch.add("MWL", "8B 43 6C 89 05", [](memory::handle ptr) {
			g_pointers->m_gta.m_max_wanted_level = ptr;
		});

		// Blame Explode
		main_batch.add("BE", "0F 85 ? ? ? ? 48 8B 05 ? ? ? ? 48 8B 48 08 E8", [](memory::handle ptr) {
			g_pointers->m_gta.m_blame_explode = ptr;
		});

		//Patch blocked explosions
		main_batch.add("EP", "E8 ? ? ? ? 48 8D 4C 24 20 E8 ? ? ? ? 4C 8D 9C 24 80 01 00 00", [](memory::handle ptr) {
			g_pointers->m_gta.m_explosion_patch = ptr;
		});

		// Is Matchmaking Session Valid
		main_batch.add("IMSV", "48 89 5C 24 08 48 89 6C 24 10 48 89 74 24 18 57 41 54 41 55 41 56 41 57 48 83 EC 20 45 0F", [](memory::handle ptr) {
			g_pointers->m_gta.m_is_matchmaking_session_valid = ptr;
		});

		// Broadcast Net Array Patch
		main_batch.add("BP", "74 73 FF 90 ? ? ? ? 8B D5 4C 8B 00 48 8B C8 41 FF 50 30", [](memory::handle ptr) {
		...
		main_batch.add("SOD", "66 45 3B C1 74 38", [](memory::handle ptr) {
			g_pointers->m_gta.m_sound_overload_detour = ptr;
		});

		// Disable Collision
		main_batch.add("DC", "48 8B D1 49 8B CA ? ? ? ? ? 48 8B D1 49 8B CA", [](memory::handle ptr) {
			g_pointers->m_gta.m_disable_collision = ptr;
		});

		// Crash Trigger
		main_batch.add("CT", "48 3B F8 74 ? 8B 1D", [](memory::handle ptr) {
			g_pointers->m_gta.m_crash_trigger = ptr;
		});

		// Script VM Patch 1
		main_batch.add("SVM1", "3b 0a 0f 83 ? ? ? ? 48 ff c7", [](memory::handle ptr) {
			g_pointers->m_gta.m_script_vm_patch_1 = ptr;
		});

		// Script VM Patch 2
		main_batch.add("SVM2", "3b 0a 0f 83 ? ? ? ? 49 03 fa", [](memory::handle ptr) {
			g_pointers->m_gta.m_script_vm_patch_2 = ptr;
		});

		// Script VM Patches 3 and 4
		main_batch.add("SVM3&4", "3b 11 0f 83 ? ? ? ? 48 ff c7", [](memory::handle ptr) {
			g_pointers->m_gta.m_script_vm_patch_3 = ptr;
			g_pointers->m_gta.m_script_vm_patch_4 = ptr.add(0x1C);
		});

		// Script VM Patches 5 and 6
		main_batch.add("SVM5&6", "3b 11 0f 83 ? ? ? ? 49 03 fa", [](memory::handle ptr) {
		...
		// Blip List
		main_batch.add("BLPLST", "4C 8D 05 ? ? ? ? 0F B7 C1", [](memory::handle ptr) {
			g_pointers->m_gta.m_blip_list = ptr.add(3).rip().as<CBlipList*>();
		});

		// TimecycleKeyframeData
		main_batch.add("TCYCL", "48 83 EC 18 48 8B 0D", [](memory::handle ptr) {
			g_pointers->m_gta.m_timecycle_keyframe_override = ptr.as<PVOID>();
		});

		// Free Event Error
		main_batch.add("FEE", "48 8B 5C 24 40 48 8B 6C 24 48 48 8B 74 24 50 48 8B 7C 24 58 48 83 C4 30 41 5E C3 48 8B 0D", [](memory::handle ptr) {
			g_pointers->m_gta.m_free_event_error = ptr.add(0x31).as<PVOID>();
		});

		// Activate Special Ability Patch
		main_batch.add("ASAP", "48 89 5C 24 08 48 89 74 24 10 57 48 83 EC 20 0F B7 49 30 48 8B F2 E8 ? ? ? ? 33 DB 48 8B F8 48 85 C0 74 35", [](memory::handle ptr) {
			g_pointers->m_gta.m_activate_special_ability_patch = ptr.as<PVOID>();
		});

		// ClearDecals
		main_batch.add("DCLMGR", "48 8D 0D ? ? ? ? 41 83 C8 FF E8 ? ? ? ? 4C", [](memory::handle ptr) {
		...
		main_batch.add("ISCOA", "88 1D ? ? ? ? E8 ? ? ? ? 33 F6", [](memory::handle ptr) {
			g_pointers->m_gta.m_is_social_club_overlay_active = ptr.add(2).rip().as<bool*>();
		});

		// Game Skeleton
		main_batch.add("GS", "48 8D 0D ? ? ? ? BA ? ? ? ? 74 05 BA ? ? ? ? E8 ? ? ? ? E8 ? ? ? ? C6 05 ? ? ? ? ? 48 8D 0D ? ? ? ? BA ? ? ? ? 84 DB 75 05 BA ? ? ? ? E8 ? ? ? ? 48 8B CD C6 05 ? ? ? ? ? E8 ? ? ? ? 84", [](memory::handle ptr) {
		...
		// Get Ped Bone
		main_batch.add("GPB", "48 89 5C 24 ?? 48 89 6C 24 ?? 48 89 74 24 ?? 57 48 83 EC 60 48 8B 01 41 8B E8 48 8B F2", [](memory::handle ptr) {
	...
	void pointers::get_sc_batch(memory::module region)
	{
		...
		socialclub_batch.add("SGPA", "48 8B C4 48 89 58 08 48 89 68 10 48 89 70 18 48 89 78 20 41 54 41 56 41 57 48 83 EC 40 33 DB 49", [](memory::handle ptr) {
