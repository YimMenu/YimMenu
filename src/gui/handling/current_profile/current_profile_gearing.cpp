...
namespace big
{
	void tab_current_profile::tab_gearing()
	{
		...
		{
			ImGui::TextUnformatted("HANDLING_INITIAL_DRIVE_GEARS"_T.data());
			...
			ImGui::TextUnformatted("HANDLING_UPSHIFT_MULTIPLIER"_T.data());
			ImGui::DragFloat("###handling_upshift", &g_local_player->m_vehicle->m_handling_data->m_upshift, .01f, 0.f, 10.f);

			ImGui::TextUnformatted("HANDLING_DOWNSHIFT_MULTIPLIER"_T.data());
