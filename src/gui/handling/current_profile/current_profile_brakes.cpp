...
namespace big
{
	void tab_current_profile::tab_brakes()
	{
		...
		{
			ImGui::TextUnformatted("HANDLING_BRAKE_FORCE"_T.data());
			ImGui::DragFloat("##brake force", &g_local_player->m_vehicle->m_handling_data->m_brake_force, .01f, 0.f, 10.f);

			ImGui::TextUnformatted("HANDLING_BRAKE_BIAS"_T.data());
			...
			ImGui::TextUnformatted("HANDLING_HAND_BRAKE_FORCE"_T.data());
