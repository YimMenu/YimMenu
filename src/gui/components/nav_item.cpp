...
namespace big
{
	void components::nav_item(std::pair<tabs, navigation_struct>& navItem, int nested)
	{
		...
		if (curTab && !navItem.second.sub_nav.empty())
		{
			...
			{
				dl->AddRectFilled({ 10.f, ImGui::GetCursorPosY() + (100.f * g.window.gui_scale) }, {(10.f + (300.f * g.window.gui_scale)), (ImGui::GetCursorPosY() + (100.f * (g.window.gui_scale)) + ImGui::CalcTextSize("A").y + (ImGui::GetStyle().ItemInnerSpacing.y / g.window.gui_scale) * 2) }, ImGui::ColorConvertFloat4ToU32({1.f, 1.f, 1.f, .15f + (.075f * nested)}));
