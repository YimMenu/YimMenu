...
namespace big
{
	void view::teleport()
	{
		...
		ImGui::Text(fmt::vformat("IPL_CNT"_T, fmt::make_format_args(selected_ipl.ipl_names.size())).data());
		ImGui::Text(fmt::vformat("IPL_POSITION"_T,
		    fmt::make_format_args(selected_ipl.location.x, selected_ipl.location.y, selected_ipl.location.z))
		                .data());
	}
}
