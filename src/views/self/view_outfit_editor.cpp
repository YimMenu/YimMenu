...
namespace big
{
	void view::outfit_editor()
	{
		...
		for (auto& item : components.items)
		{
			...
			if (ImGui::InputInt(fmt::format("{} [0,{}]", item.label, item.drawable_id_max).c_str(), &item.drawable_id, 0))
			{
		...
		{
			ImGui::SetNextItemWidth(60);
			if (ImGui::InputInt(fmt::format("{} {} [0,{}]", item.label, "OUTFIT_TEX"_T, item.texture_id_max).c_str(), &item.texture_id, 0))
			{
				g_fiber_pool->queue_job([item] {
