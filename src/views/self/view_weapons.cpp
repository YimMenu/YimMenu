...
namespace big
{
	void view::weapons() 
	{
		...
		ImGui::Separator();
		components::sub_title("Weapon Mods");
		...
		ImGui::BeginGroup();
		{
			...
			{
				Hash weaponHash;
				WEAPON::GET_CURRENT_PED_WEAPON(self::ped, &weaponHash, 1); //Should have a global var for this
		...
		//If I put the code below in a group to make it consitient like the rest of them I get some dumb
		//compiler error that I don't care to fix so it's just not going in a group
		...
		case CustomWeapon::VEHICLE_GUN:
			...
			// idk it just looks like the rest of the solution to me
			static char vehicle_gun[12];
			std::memcpy(vehicle_gun, g.weapons.vehicle_gun_model.c_str(), 12); //Hardcoded int?
