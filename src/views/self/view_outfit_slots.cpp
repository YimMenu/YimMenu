	...
	void view::outfit_slots()
	{
		if (*g_pointers->m_gta.m_script_globals)
		{
			...
			for (auto& item : components.items)
			{
				...
				ImGui::InputInt(fmt::format("{} [0,{}]##1", item.label, item.drawable_id_max).c_str(), outfit::get_component_drawable_id_address(slot, item.id));
			}
			ImGui::EndGroup();
			...
			{
				ImGui::SetNextItemWidth(120);
				ImGui::InputInt(fmt::format("{} {} [0,{}]##2", item.label, "OUTFIT_TEX"_T, item.texture_id_max).c_str(), outfit::get_component_texture_id_address(slot, item.id));
			}
			ImGui::EndGroup();
			...
			{
				ImGui::SetNextItemWidth(120);
				ImGui::InputInt(fmt::format("{} [0,{}]##3", item.label, item.drawable_id_max).c_str(), outfit::get_prop_drawable_id_address(slot, item.id));
			}
			ImGui::EndGroup();
			...
			{
				ImGui::SetNextItemWidth(120);
				ImGui::InputInt(fmt::format("{} {} [0,{}]##4", item.label, "OUTFIT_TEX"_T, item.texture_id_max).c_str(), outfit::get_prop_texture_id_address(slot, item.id));
			}
			ImGui::EndGroup();
		}
	}
