	...
	void view::outfit_slots()
	{
		if (*g_pointers->m_gta.m_script_globals)
		{
			...
			for (auto& item : components.items)
			{
				...
				ImGui::InputInt(fmt::format("{} [0,{}]", item.label, item.drawable_id_max).c_str(), outfit::get_component_drawable_id_address(slot, item.id), 0);
			}
			ImGui::EndGroup();
			...
			{
				ImGui::SetNextItemWidth(60);
				ImGui::InputInt(fmt::format("{} {} [0,{}]", item.label, "OUTFIT_TEX"_T, item.texture_id_max).c_str(), outfit::get_component_texture_id_address(slot, item.id), 0);
			}
			ImGui::EndGroup();
			...
			{
				ImGui::SetNextItemWidth(60);
				ImGui::InputInt(fmt::format("{} [0,{}]", item.label, item.drawable_id_max).c_str(), outfit::get_prop_drawable_id_address(slot, item.id), 0);
			}
			ImGui::EndGroup();
			...
			{
				ImGui::SetNextItemWidth(60);
				ImGui::InputInt(fmt::format("{} {} [0,{}]", item.label, "OUTFIT_TEX"_T, item.texture_id_max).c_str(), outfit::get_prop_texture_id_address(slot, item.id), 0);
			}
			ImGui::EndGroup();
		}
	}
