...
namespace big
{
	void view::self()
	{
		...
		components::command_button<"suicide">();		ImGui::SameLine();
		components::command_button<"heal">();		ImGui::SameLine();
		components::command_button<"fillsnacks">();		ImGui::SameLine();
		components::command_button<"skipcutscene">();		ImGui::SameLine();
		components::command_button<"clean">();

		ImGui::Separator(); //No line break here. Same feature category.

		ImGui::BeginGroup();
		{
			ImGui::Checkbox("God Mode", &g.self.god_mode); //Why is this special
			//components::command_checkbox<"otr">(); Moved to be in mobile tab only
		...
		ImGui::BeginGroup();
		{
			...
			ImGui::Checkbox("No Idle Kick", &g.tunables.no_idle_kick); //Why is this special
			components::command_checkbox<"walkunder">();

			//The way these two features look on the menu is dumb
		...
		ImGui::BeginGroup();
		{
			...
			if (!g.self.never_wanted)
			{
				...
				if (ImGui::SliderInt("###wanted_level", &g.self.wanted_level, 0, 5)
		...
		ImGui::BeginGroup();
		{
			ImGui::Checkbox("Hide Radar", &g.self.hide_radar);			ImGui::SameLine();
			ImGui::Checkbox("Hide Ammo", &g.self.hide_ammo);			ImGui::SameLine();

			ImGui::Checkbox("Force show HUD", &g.self.force_show_hud);

			ImGui::Combo("##hud_comp_combo", &g.self.selected_hud_component, hud_component_names, (int)HudComponents::HUD_WEAPONS);		ImGui::SameLine();

			components::button("Hide", [] {g.self.hud_components_states[g.self.selected_hud_component] = true; });			ImGui::SameLine();
			components::button("Show", [] {g.self.hud_components_states[g.self.selected_hud_component] = false; });
