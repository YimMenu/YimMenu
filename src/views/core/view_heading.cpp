...
namespace big
{
	void view::heading()
	{
		...
		if (ImGui::Begin("menu_heading", nullptr, window_flags | ImGuiWindowFlags_NoScrollbar))
		{
			...
			ImGui::EndGroup();
			ImGui::SameLine();
			ImGui::SetCursorPos({ 300.f - ImGui::CalcTextSize("U").x - ImGui::GetStyle().ItemSpacing.x, ImGui::GetStyle().WindowPadding.y / 2 + ImGui::GetStyle().ItemSpacing.y + (ImGui::CalcTextSize("W").y / 2) });
