		...
		if (ImGui::BeginTabItem("DEBUG_TAB_GLOBALS"_T.data()))
		{
			...
			for (int i = 0; i < global_test.global_appendages.size(); i++)
			{
				...
				case GlobalAppendageType_At:
					ImGui::SetNextItemWidth(200.f);
					ImGui::InputScalar(fmt::format("At##{}{}", i, (int)item.type).c_str(),
					    ImGuiDataType_S64,
					    &global_test.global_appendages[i].index);
					ImGui::SameLine();
					ImGui::SetNextItemWidth(200.f);
					ImGui::InputScalar(fmt::format("Size##{}{}", i, (int)item.type).c_str(),
					    ImGuiDataType_S64,
				...
				case GlobalAppendageType_PlayerId:
					ImGui::SetNextItemWidth(200.f);
					ImGui::InputScalar(fmt::format("Read Player ID Size##{}{}", i, (int)item.type).c_str(),
					    ImGuiDataType_S64,
					    &global_test.global_appendages[i].size);
					break;
				}
