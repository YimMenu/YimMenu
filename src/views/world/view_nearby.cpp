...
namespace big
{
	void view::nearby()
	{
		...
		components::button<ImVec2(110, 0)>
			("Delete", [] { 
				for (auto peds : entity::get_entities(false, true)) entity::delete_entity(peds); 
			}); ImGui::SameLine();
		components::button<ImVec2(110, 0), ImVec4(0.70196f, 0.3333f, 0.00392f, 1.f)>
			("Kill", [] { 
				for (auto peds : entity::get_entities(false, true)) ped::kill_ped(peds); 
			}); ImGui::SameLine();
		components::button<ImVec2(110, 0), ImVec4(0.76078f, 0.f, 0.03529f, 1.f)>
			("Kill Enemies", [] { 
				for (auto peds : entity::get_entities(false, true)) ped::kill_ped_by_relation(peds, 4 || 5); 
			});
