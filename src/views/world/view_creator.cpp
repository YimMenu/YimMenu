...
namespace big
{
	void view::creator()
	{
		...
		{
			components::button(
			    "CREATOR_SAVE_TO_FILE"_T, [] { creator_storage_service::save_file(selected_creator_file); });

			ImGui::SameLine();

			components::button(
			    "CREATOR_LOAD_FROM_FILE"_T, [] { creator_storage_service::load_file(selected_creator_file); });
		}
		...
		components::button("REFRESH"_T, [] { cached_creator_files = false; });
				            ...
				            [content_id]
				            {
					            if (NETWORK::UGC_QUERY_BY_CONTENT_ID(content_id.c_str(), false, "gta5mission"))
					            {
						            ...
						            if (g_api_service->download_job_metadata(content_id, f1 < 0 ? 0 : f1, f0 < 0 ? 0 : f0, NETWORK::UGC_GET_CONTENT_LANGUAGE(0)))
						            {
							            ...
							            g_notification_service->push(
							                "CREATOR_JOB_IMPORT_NOTIFICATION"_T.data(), "CREATOR_JOB_IMPORT_SUCCESS"_T.data());
						            }
						            else
						            {
					            ...
					            else
					            {
						            g_notification_service->push_error(
						                "CREATOR_JOB_IMPORT_NOTIFICATION"_T.data(), "CREATOR_JOB_UGC_QUERY_FAILED"_T.data());
					            }
				            });
			        });
		    });
		...
		ImGui::BeginGroup();
		components::button("RACE"_T, [] { scripts::start_creator_script(RAGE_JOAAT("fm_race_creator")); });
		ImGui::SameLine();
		components::button("CAPTURE"_T, [] { scripts::start_creator_script(RAGE_JOAAT("fm_capture_creator")); });
		ImGui::SameLine();
		components::button("DEATHMATCH"_T, [] { scripts::start_creator_script(RAGE_JOAAT("fm_deathmatch_creator")); });
		ImGui::SameLine();
