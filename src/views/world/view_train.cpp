...
namespace big
{
	void view::train()
	{
		components::button("HIJACK_TRAIN"_T, [] { train::hijack_train(); });

		ImGui::SameLine();

		components::button("DELETE_TRAIN"_T, [] { train::delete_train(); });
		...
		components::button("EXIT_TRAIN"_T, [] { train::exit_train(); });
