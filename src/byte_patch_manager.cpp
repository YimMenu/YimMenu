...
namespace big
{
	static void init()
	{
		...
		toxic::explosion_anti_cheat_bypass::m_can_blame_others =
		    memory::byte_patch::make(g_pointers->m_explosion_patch.as<std::uint16_t*>(), 0xE990).get();
		...
		memory::byte_patch::make(g_pointers->m_is_matchmaking_session_valid.as<void*>(), std::to_array({0xB0, 0x01, 0xC3}))
		...
		// NTQVM Caller
		memory::byte_patch::make(g_pointers->m_ntqvm_caller.add(4).rip().sub(32).as<uint64_t*>(), (uint64_t)&hooks::nt_query_virtual_memory)
		    ->apply();
