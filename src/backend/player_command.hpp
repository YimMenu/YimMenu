...
namespace big
{
	...
	public:
		player_all_component(player_command* parent, const std::string& name, const std::string& label, const std::string& description, std::uint8_t num_args);
	};
	...
	public:
		...
		player_command(const std::string& name, const std::string& label, const std::string& description, std::uint8_t num_args, bool make_all_version = true);
	};
}