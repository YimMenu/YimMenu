...
namespace big
{
	void register_script_patches()
	{
		...
		g_script_patcher_service->add_patch({ RAGE_JOAAT("freemode"), "2D 01 08 00 00 5D ? ? ? 56 ? ? 71", 5, { 0x2E, 0x01, 0x00 }, &g->tunables.no_idle_kick });
		...
		g_script_patcher_service->add_patch({ RAGE_JOAAT("shop_controller"), "2D 01 04 00 00 2C ? ? ? 56 ? ? 71", 5, { 0x6E, 0x2E, 0x01, 0x01 }, nullptr }); // despawn bypass
