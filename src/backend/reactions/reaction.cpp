...
namespace big
{
	...
	void reaction::process_common(player_ptr player)
	{
		...
		if (log)
		{
			...
			LOG(WARNING) << fmt::format("Received {} from {} ({})", m_event_name, player->get_name(), rockstar_id);
		}
	...
	void reaction::process(player_ptr player)
	{
		...
		if (announce_in_chat)
		{
			...
			{
				char chat[255];
				snprintf(chat, sizeof(chat), fmt::format("{} {}", g.session.chat_output_prefix, m_announce_message).data(), player->get_name());
