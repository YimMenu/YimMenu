...
namespace big
{
	std::optional<std::vector<std::uint64_t>> send_to_interior::parse_args_p(const std::vector<std::string>& args, const std::shared_ptr<command_context> ctx)
	{
		return std::vector<std::uint64_t>{(uint64_t)std::atoi(args[0].c_str())};
	}

	CommandAccessLevel send_to_interior::get_access_level()
	{
		return CommandAccessLevel::AGGRESSIVE;
	}

	void send_to_interior::execute(player_ptr player, const std::vector<std::uint64_t>& _args, const std::shared_ptr<command_context> ctx)
	{
		float max               = 1e+38f;
