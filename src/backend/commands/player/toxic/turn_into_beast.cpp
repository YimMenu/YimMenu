...
namespace big
{
	CommandAccessLevel turn_into_beast::get_access_level()
	{
		return CommandAccessLevel::AGGRESSIVE;
	}

	void turn_into_beast::execute(player_ptr player, const std::vector<std::uint64_t>& _args, const std::shared_ptr<command_context> ctx)
	{
		auto id = player->id();
	...
	CommandAccessLevel turn_into_beast_all::get_access_level()
	{
		return CommandAccessLevel::AGGRESSIVE;
	}

	void turn_into_beast_all::execute(const std::vector<std::uint64_t>& _args, const std::shared_ptr<command_context> ctx)
	{
		scripts::start_launcher_script(47);
