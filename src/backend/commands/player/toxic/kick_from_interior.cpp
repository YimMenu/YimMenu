				...
				g_fiber_pool->queue_job(
				    [id]
				    {
					    ...
					    gta_util::execute_as_script(script,
					        [instance]
					        {
						        if (auto hook = g_hooking
						                            ->m_handler_hooks[(CGameScriptHandler*)rage::scrThread::get()->m_handler]
						                            .get())
						        {
							        hook->disable();
					    ...
					    for (int i = 0; i < 100; i++)
					    {
						    ...
						    gta_util::execute_as_script(
						        script, [&status] { status = NETWORK::NETWORK_GET_SCRIPT_STATUS(); });
					    ...
					    for (int i = 0; i < 3600; i++)
					    {
						    bool received = false;
						    gta_util::execute_as_script(
						        script, [&received] { received = NETWORK::NETWORK_HAS_RECEIVED_HOST_BROADCAST_DATA(); });
