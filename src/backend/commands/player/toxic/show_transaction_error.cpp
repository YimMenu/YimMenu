...
namespace big
{
	class show_transaction_error : player_command
	{
		...
		virtual void execute(player_ptr player, const std::vector<std::uint64_t>& _args, const std::shared_ptr<command_context> ctx)
		{
			...
			int64_t args[arg_count] = {(int64_t)eRemoteEvent::TransactionError, (int64_t)self::id, 1, 0, 0,
			    scr_globals::gpbd_fm_3.as<GPBD_FM_3*>()->Entries[player->id()].ScriptEventReplayProtectionCounter,
			    *scr_globals::gsbd_fm_events.at(9).as<int*>(), *scr_globals::gsbd_fm_events.at(10).as<int*>()};
