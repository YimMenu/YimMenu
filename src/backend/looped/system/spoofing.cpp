	...
	void looped::system_spoofing()
	{
		...
		if (*g_pointers->m_gta.m_is_session_started)
		{
			gta_util::execute_as_script(RAGE_JOAAT("freemode"), [] {
				if (NETWORK::NETWORK_GET_THIS_SCRIPT_IS_NETWORK_SCRIPT() && NETWORK::NETWORK_GET_SCRIPT_STATUS() == 2 && NETWORK::NETWORK_HAS_RECEIVED_HOST_BROADCAST_DATA())
				{
					...
					if (g.spoofing.spoof_blip && !STREAMING::IS_PLAYER_SWITCH_IN_PROGRESS())
					{
