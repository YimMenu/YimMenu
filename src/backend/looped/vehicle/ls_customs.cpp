...
namespace big
{
	...
	void looped::vehicle_ls_customs()
	{
		if (g.vehicle.ls_customs && g.vehicle.ls_customs != bLastLsCustoms)
		{
			g_fiber_pool->queue_job([] {
				...
				if (scripts::wait_till_loaded(RAGE_JOAAT("carmod_shop")))
				{
					...
					*script_local(g.m_modshop_thread->m_stack, 733).at(446).as<int*>() = 2;
					*script_local(g.m_modshop_thread->m_stack, 1865).as<ControllerInputs*>() = ControllerInputs::INPUT_FRONTEND_LT;
				}
			});
			bLastLsCustoms = true;
		}
		...
		{
			*script_local(g.m_modshop_thread->m_stack, 2046).as<bool*>()           = false;
			*script_local(g.m_modshop_thread->m_stack, 733).at(638).as<int*>()     = -1;
			*script_local(g.m_modshop_thread->m_stack, 733).at(409).as<Vehicle*>() = self::veh;

			if (*script_local(g.m_modshop_thread->m_stack, 733).at(446).as<int*>() == 0)
				*script_local(g.m_modshop_thread->m_stack, 733).at(446).as<int*>() = 2;
