...
namespace big::toxic
{
	...
	inline void kick_player_from_interior(player_ptr target)
	{
		...
		int64_t args[arg_count]{
			...
			(int64_t)self::id,
			*scr_globals::globalplayer_bd.at(target->id(), scr_globals::size::globalplayer_bd).at(317).at(6).as<int64_t*>(),
			*scr_globals::globalplayer_bd.at(target->id(), scr_globals::size::globalplayer_bd).at(317).at(7).as<int64_t*>(),
		};
