...
namespace big::globals
{
	inline void clear_wanted_player(Player target)
	{
		...
		int64_t args[arg_count]    = {static_cast<int64_t>(eRemoteEvent::ClearWantedLevel), self::id,
	...
	inline void give_remote_otr(Player target)
	{
		...
		int64_t args[arg_count]    = {static_cast<int64_t>(eRemoteEvent::RemoteOffradar), (int64_t)self::id, (int64_t)(NETWORK::GET_NETWORK_TIME() + 1), 0, true, false,
