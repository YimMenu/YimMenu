...
namespace big
{
	...
	void scan_explosion_event(CNetGamePlayer* player, rage::datBitBuffer* buffer)
	{
		...
		if (entity && entity->gap28 == 4 && reinterpret_cast<CPed*>(entity)->m_player_info && player->m_player_info->m_ped && player->m_player_info->m_ped->m_net_object && ownerNetId != player->m_player_info->m_ped->m_net_object->m_object_id)
		{
