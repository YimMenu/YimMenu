...
namespace big
{
	...
	bool hooks::receive_net_message(void* netConnectionManager, void* a2, rage::netConnection::InFrame* frame)
	{
			...
			(msgType == rage::eNetMessage::MsgTransitionLaunch && frame->m_connection_identifier != gta_util::get_network()->m_transition_session.m_connection_identifier))
		{
			if (player)
			{
				g_notification_service->push_error("Protections", fmt::format("Blocked invalid transition launch crash from {}", player->get_name()));
			}
			else
			{
		...
		if (player)
		{
			switch (msgType)
			{
				...
				case rage::eNetMessage::MsgRequestObjectIds:
				{
					...
					{
						g_notification_service->push("Join Blocker", fmt::format("Trying to prevent {} from joining...", player->get_name()));
						return true;
					}
					break;
				}
			...
			switch (msgType)
			{
				case rage::eNetMessage::MsgLostConnectionToHost:
				{
					...
					for (auto& [_, plyr] : g_player_service->players())
					{
						...
						{
							g_notification_service->push_error("Warning!", fmt::format("Someone tried to lost connection kick {} remotely!", plyr->get_name()));
							return true;
						}
					}
				...
				case rage::eNetMessage::MsgRemoveGamersFromSessionCmd:
				{
					...
					if (target && count == 1 && frame->m_msg_id == -1)
					{
						if (target->id() == g_player_service->get_self()->id())
							...
							g_notification_service->push_error("Warning!", fmt::format("Someone tried to breakup kick {} remotely!", target->get_name()));
					}
