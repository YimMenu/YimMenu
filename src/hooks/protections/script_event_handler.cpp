...
namespace big
{
	...
	bool hooks::scripted_game_event(CScriptedGameEvent* scripted_game_event, CNetGamePlayer* player)
	{
		...
		case eRemoteEvent::Bounty:
			...
			{
				g.reactions.bounty.process(plyr, false);
				return true;
			}
			break;
		case eRemoteEvent::CeoKick:
			...
			{
				g.reactions.ceo_kick.process(plyr, false);
				return true;
			}
			break;
		case eRemoteEvent::CeoMoney:
			...
			{
				g.reactions.ceo_money.process(plyr, false);
				return true;
			}
			break;
		case eRemoteEvent::ClearWantedLevel:
			...
			{
				g.reactions.clear_wanted_level.process(plyr, false);
				return true;
			}
			break;
		case eRemoteEvent::Crash: g.reactions.crash.process(plyr, false); return true;
		case eRemoteEvent::Crash2:
			if (args[2] > 32) // actual crash condition is if args[2] is above 255
			{
				g.reactions.crash.process(plyr, false);
				return true;
			}
			break;
		...
		case eRemoteEvent::Notification:
		{
			...
			case eRemoteEvent::NotificationMoneyStolen: g.reactions.fake_deposit.process(plyr, false); return true;
		...
		case eRemoteEvent::ForceMission:
			...
			{
				g.reactions.force_mission.process(plyr, false);
				return true;
			}
			break;
		case eRemoteEvent::GiveCollectible:
			...
			{
				g.reactions.give_collectible.process(plyr, false);
				return true;
			}
			break;
		case eRemoteEvent::GtaBanner:
			...
			{
				g.reactions.gta_banner.process(plyr, false);
				return true;
			}
			break;
		case eRemoteEvent::MCTeleport:
			if (g.protections.script_events.mc_teleport && args[3] <= 32 && !is_player_our_boss(plyr->id()))
			{
				for (int i = 0; i < 32; i++)
				{
					...
					{
						g.reactions.mc_teleport.process(plyr, false);
						return true;
					}
				}
			}
		...
		case eRemoteEvent::PersonalVehicleDestroyed:
			...
			{
				g.reactions.personal_vehicle_destroyed.process(plyr, false);
				return true;
			}
			break;
		case eRemoteEvent::RemoteOffradar:
			...
			{
				g.reactions.remote_off_radar.process(plyr, false);
				return true;
			}
			break;
		case eRemoteEvent::TSECommand:
			...
			{
				g.reactions.rotate_cam.process(plyr, false);
				return true;
			}
			break;
		case eRemoteEvent::SendToCayoPerico:
			...
			{
				g.reactions.send_to_location.process(plyr, false);
				return true;
			}
			break;
		case eRemoteEvent::SendToCutscene:
			...
			{
				g.reactions.send_to_cutscene.process(plyr, false);
				return true;
			}
			break;
		case eRemoteEvent::SendToLocation:
		{
			...
			if (!known_location)
			{
				g.reactions.tse_freeze.process(plyr, false);
				return true;
			}
			break;
		}
		...
		case eRemoteEvent::Spectate:
			...
			{
				g.reactions.spectate_notification.process(plyr, false);
				return true;
			}
			break;
		case eRemoteEvent::Teleport:
			...
			{
				g.reactions.force_teleport.process(plyr, false);
				return true;
			}
			break;
		case eRemoteEvent::TransactionError: g.reactions.transaction_error.process(plyr, false); return true;
		case eRemoteEvent::VehicleKick:
			if (g.protections.script_events.vehicle_kick)
			{
				g.reactions.vehicle_kick.process(plyr, false);
				return true;
			}
			break;
		case eRemoteEvent::NetworkBail:
			session::add_infraction(plyr, Infraction::TRIED_KICK_PLAYER);
			g.reactions.network_bail.process(plyr, false);
			return true;
		case eRemoteEvent::TeleportToWarehouse:
			...
			{
				g.reactions.teleport_to_warehouse.process(plyr, false);
				return true;
			}
			break;
		case eRemoteEvent::StartActivity:
		{
			...
			if (g.protections.script_events.start_activity)
			{
				...
				{
					g.reactions.start_activity.process(plyr, false);
					return true;
				}
				...
				{
					g.reactions.trigger_business_raid.process(plyr, false);
					return true;
				}
			}
		...
		case eRemoteEvent::InteriorControl:
		{
			...
			if (interior < 0 || interior > 161) // the upper bound will change after an update
			{
				if (auto plyr = g_player_service->get_by_id(player->m_player_id))
					session::add_infraction(plyr, Infraction::TRIED_KICK_PLAYER);
				g.reactions.null_function_kick.process(plyr, false);
				return true;
			}
			...
			{
				// g.reactions.send_to_interior.process(plyr, false); false positives
		...
		case eRemoteEvent::DestroyPersonalVehicle:
			g.reactions.destroy_personal_vehicle.process(plyr, false);
			return true;
		case eRemoteEvent::KickFromInterior:
			...
			{
				g.reactions.kick_from_interior.process(plyr, false);
				return true;
			}
			break;
		...
		case eRemoteEvent::StartScriptProceed:
		{
			...
			if (auto script = gta_util::find_script_thread(RAGE_JOAAT("freemode")))
			{
				if (script->m_net_component && ((CGameScriptHandlerNetComponent*)script->m_net_component)->m_host
				    && ((CGameScriptHandlerNetComponent*)script->m_net_component)->m_host->m_net_game_player != player)
				{
					g.reactions.start_script.process(plyr, false);
					return true;
				}
			}
			break;
		...
		if (*(int*)&args[1] != player->m_player_id && player->m_player_id != -1)
		{
			...
			g.reactions.tse_sender_mismatch.process(plyr, false);
			return true;
		}
