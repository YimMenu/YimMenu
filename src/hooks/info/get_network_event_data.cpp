	...
	void hooks::get_network_event_data(int64_t unk, rage::CEventNetwork* net_event)
	{
		switch (net_event->get_type()) 
		{
			...
			case 186: // CEventNetworkEntityDamage
			{
				...
				if (auto damager = (rage::CDynamicEntity*)g_pointers->m_get_script_handle(damage_data.m_damager_index); damager && damager->gap28 == 4 && reinterpret_cast<CPed*>(damager)->m_player_info)
				{
					...
					{
						if (auto victim = (rage::CDynamicEntity*)g_pointers->m_get_script_handle(damage_data.m_victim_index); victim && victim->gap28 == 4 && reinterpret_cast<CPed*>(victim)->m_player_info)
						{
