	...
	public:
		...
		s_context_menu shared_menu{ContextEntityType::SHARED,
		    ...
		    {{"EXPLODE",
		        ...
		        {"COPY HASH", [this] {
			         ImGui::SetClipboardText(fmt::format("0x{:08X}", (rage::joaat_t)m_pointer->m_model_info->m_hash).c_str());
			         g_notification_service->push("Context Menu",
			             fmt::format("Copy hash 0x{:08X}", (rage::joaat_t)m_pointer->m_model_info->m_hash).c_str());
		         }}}};
