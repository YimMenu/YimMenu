...
namespace big
{
	void view::lsc()
	{
		...
		if (
			...
			(color_to_change == 8 && !owned_mods[MOD_NEON_LEFT_ON] && !owned_mods[MOD_NEON_RIGHT_ON] && !owned_mods[MOD_NEON_FRONT_ON] && !owned_mods[MOD_NEON_BACK_ON])
		) {
			color_to_change = 0;
			color_type = 8;
		}
		...
		if (ImGui::ListBoxHeader("##color_options", ImVec2(120, 254)))
		{
			...
			if (!owned_mods[MOD_NEON_LEFT_ON] && !owned_mods[MOD_NEON_RIGHT_ON] && !owned_mods[MOD_NEON_FRONT_ON] && !owned_mods[MOD_NEON_BACK_ON])
			{
				ImGui::BeginDisabled();
			}
