...
namespace big
{
	void view::heading()
	{
		ImGui::SetNextWindowSize({ (g->window.x / 2554.f) * 250.f, 80.f });
		...
		if (ImGui::Begin("menu_heading", nullptr, window_flags | ImGuiWindowFlags_NoScrollbar))
		{
			...
			ImGui::EndGroup();
			ImGui::SameLine();
			ImGui::SetCursorPos({ (g->window.x / 2554.f) * 250.f - ImGui::CalcTextSize("Unload").x - ImGui::GetStyle().ItemSpacing.x, ImGui::GetStyle().WindowPadding.y / 2 + ImGui::GetStyle().ItemSpacing.y + (ImGui::CalcTextSize("W").y / 2) });
