...
namespace big
{
	...
	bool anti_cheat_service::mark_as_modder(Player player, int score, std::string detections)
	{
		...
		m_modders.push_back({ cplayer->get_name(), cplayer->get_net_data()->m_rockstar_id2, score, detections });
	...
	int anti_cheat_service::modder_check(Player player)
	{
		...
		{
			int current_player = get_moddb_player_from_rid(net_player_data->m_rockstar_id2);
			int corrent_score = m_modders[current_player].score;
			if (corrent_score < score)
			{
				add_score_to_modder(net_player_data->m_rockstar_id2, score - corrent_score, detections);
			}
		}
